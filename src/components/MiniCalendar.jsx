...
export default function MiniCalendar({ monthDate, onChangeMonth, onSelectDate, selectedDate, monthTasks }) {
    ...
    <div>
      <div className="cal-header">
        ...
        <div className="cal-nav">
          <button onClick={() => onChangeMonth(subMonths(monthDate, 1))}>&#8249;</button>
          <button onClick={() => onChangeMonth(addMonths(monthDate, 1))}>&#8250;</button>
        </div>
      </div>
      <div className="cal-grid">
        ...
        {days.map((d, i) => {
          ...
          return (
            <div key={i} className={'cal-day' + (isOther ? ' other-month' : '') + (isTd ? ' today' : '') + (isSel ? ' selected' : '')}
