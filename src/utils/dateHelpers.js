import { format, startOfMonth, endOfMonth, startOfWeek, endOfWeek, eachDayOfInterval, isSameMonth, isSameDay, isToday, addMonths, subMonths, subDays, parseISO } from 'date-fns';
...
export function calcStreak(tasks) {
  ...
  for (const ds of dates) {
    ...
    if (ds === expected) {
      ...
      if (dt.length > 0 && dt.every(t => t.isCompleted)) { streak++; check = subDays(check, 1); }
      else break;
...
export function getWeeklyTrends(tasks) {
  ...
  return Object.entries(weeks).map(([label, v]) => ({
    label, total: v.total, done: v.done, rate: v.total > 0 ? v.done / v.total : 0
  }));
}
...
export function getCategoryStats(tasks) {
  ...
  return Object.entries(cats).map(([name, v]) => ({
    name, total: v.total, done: v.done, rate: v.total > 0 ? v.done / v.total : 0
  }));
}

export { format, isSameMonth, isSameDay, isToday, addMonths, subMonths, parseISO };